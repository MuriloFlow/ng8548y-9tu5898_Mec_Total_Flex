...
export function FlowdeskBlockedScreen({ entitlement }: { entitlement: Entitlement | null }) {
  ...
  const hasPayOption = Boolean(payUrl);

  const title =
    hasPayOption
      ? "Aplicação bloqueada — regularize o pagamento"
      : blockType === "suspended"
        ? "Aplicação suspensa"
  ...
  const subtitle =
    hasPayOption
      ? blockType === "manual"
        ? "Há cobrança em aberto. Regularize abaixo para solicitar a liberação do acesso."
        : "O acesso está suspenso até a confirmação do pagamento abaixo."
            ...
            <div className="px-7 py-6">
              ...
              {active && active.open_amount > 0 && (
                <p className="mt-3 text-[12px] leading-relaxed text-slate-500">
                  Se o botão de pagamento não aparecer, aguarde alguns segundos ou clique em
                  verificar novamente — o link é gerado automaticamente pelo FlowDesk.
                </p>
              )}
            </div>
          )}
