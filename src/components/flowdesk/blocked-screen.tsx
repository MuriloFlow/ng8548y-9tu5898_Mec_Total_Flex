...
export function FlowdeskBlockedScreen({ entitlement }: { entitlement: Entitlement | null }) {
  ...
  const charge = entitlement?.charge ?? null;
  ...
  const check = useCallback(async (silent: boolean) => {
    ...
    try {
      ...
      // silencioso: mantém a tela de bloqueio
    } finally {
      if (!silent) setChecking(false);
    }
  }, []);

  // Libera sozinho assim que o webhook do pagamento chegar no FlowDesk.
  useEffect(() => {
    const interval = setInterval(() => void check(true), 15_000);
  ...
  return (
    <main className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden bg-slate-50 px-4 py-10">
      {/* toques sutis de gradiente ao fundo */}
      <div
        aria-hidden
      ...
      <div className="relative w-full max-w-[520px]">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1.5 text-[13.5px] leading-relaxed text-slate-600">
                ...
                <span className="font-medium text-slate-900">
                  {entitlement?.project.name ?? "sistema"}
                </span>{" "}
                está temporariamente suspenso até a regularização da cobrança abaixo.
              </p>
            </div>
          </div>
          ...
          {charge ? (
            <div className="px-7 py-6">
              ...
              <p className="mt-4 flex items-start gap-2 text-[12px] leading-relaxed text-slate-500">
                ...
                Pagamento processado pelo Mercado Pago com Pix, cartão ou boleto. A liberação é
                automática assim que a confirmação chegar — esta tela some sozinha.
              </p>
            </div>
          ) : (
            <div className="px-7 py-6">
              ...
              {entitlement && entitlement.open_amount > 0 && (
                <p className="mt-3 text-[13px] text-slate-500">
                  Total em aberto:{" "}
                  <span className="font-semibold text-slate-900">
                    {BRL.format(entitlement.open_amount)}
                  </span>{" "}
                  em {entitlement.open_invoices}{" "}
                  {entitlement.open_invoices === 1 ? "cobrança" : "cobranças"}.
                </p>
              )}
              <button
                type="button"
        ...
        <p className="mt-5 text-center text-[11.5px] text-slate-400">
          Cobrança e liberação gerenciadas pelo FlowDesk
        </p>
      </div>
    </main>
  );
